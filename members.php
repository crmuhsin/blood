...
if (isset($_POST['valueToSearch']) && $_POST['valueToSearch'] != 'All') {
	$valueToSearch = $_POST['valueToSearch'];
	$query = "SELECT * FROM (SELECT m.id, m.username, m.bloodgroup, m.age, ldon.lastdonation, m.phone FROM members m LEFT OUTER JOIN (SELECT d.member, d.place, d.lastdonation FROM donation d INNER JOIN (SELECT member, max(lastdonation) AS maxld FROM donation GROUP BY member) dm ON d.member = dm.member AND d.lastdonation = dm.maxld) AS ldon ON m.id = ldon.member) AS mem WHERE mem.bloodgroup = '$valueToSearch'";
	$search_result = mysqli_query($link, $query);
} else{
	$query = "SELECT m.id, m.username, m.bloodgroup, m.age, ldon.lastdonation, m.phone FROM members m LEFT OUTER JOIN (SELECT d.member, d.place, d.lastdonation FROM donation d INNER JOIN (SELECT member, max(lastdonation) AS maxld FROM donation GROUP BY member) dm ON d.member = dm.member AND d.lastdonation = dm.maxld) AS ldon ON m.id = ldon.member";
	$search_result = mysqli_query($link, $query);
}
            ...
            <table class="table table-bordered table-hover table-striped" id="bootstrap-table">
                ...
                <tr class="warning">
                    ...
                    <td>
                        ...
                        <a onclick="return confirm('Are you sure to Delete?');"  href="deletem.php?delid=<?php echo $mid; ?>" class="btn btn-default"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span>
                        </a>
                    </td>
                </tr>
